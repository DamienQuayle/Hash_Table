...
void HashTable::Insert(Bid bid) {
    ...
    // if there is no entry found for this key
    if (oldNode == nullptr) {
        Node* newNode = new Node(bid, key);
        nodes.insert(nodes.begin() + key, (*newNode));
    }
    else {
        // if the node is foundnode found
...
void HashTable::PrintAll() {
    // iterate through the nodes and print each one. 
    for (unsigned int i = 0; i < nodes.size(); i++) {
        Bid currentBid = nodes.at(i).bid;
        cout << currentBid.bidId << ": " << currentBid.title << " | " << currentBid.amount << " | " << currentBid.fund << endl;
    }
}
...
Bid HashTable::Search(string bidId) {
    ...
    // if no entry was found return an empty bid
    if (node == nullptr || node->key == UINT_MAX) {
        return bid;
    }

    // if there is a node found that matches key
    if (node != nullptr && node->key != UINT_MAX
        && node->bid.bidId.compare(bidId) == 0) {
        return node->bid;
    }

    // iterate through the linked list to find the match
    while (node != nullptr) {
